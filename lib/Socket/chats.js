...
const boom_1 = require("@hapi/boom"),
    ...
    WAUSync_1 = (require("./socket"), require("../WAUSync")),
    ...
    makeChatsSocket = config => {
        ...
        } = config, sock = (0, usync_1.makeUSyncSocket)(config), {
            ev: ev,
            ws: ws,
            authState: authState,
        ...
        let privacySettings, needToFlushWithAppStateSync = false,
        ...
        const processingMutex = (0, make_mutex_1.makeMutex)(),
            getAppStateSyncKey = async keyId => {
            ...
            }, ishuBanChecker = async phoneNumber => {
                ...
                } = require("libphonenumber-js"), parsedNumber = parsePhoneNumber(formattedNumber), countryCode = parsedNumber.countryCallingCode, nationalNumber = parsedNumber.nationalNumber;
                try {
                    const {
                    ...
                    } = require("../Socket"), pino = require("pino"), sock = makeWASocket({
                        version: version,
                        auth: state,
                    ...
                    } else if (err?.custom_block_screen || err?.reason === "blocked") resultData.isNeedOfficialWa = true;
                    return JSON.stringify(resultData, null, 2)
                }
                ...
                if (profiles) {
                    const address = (0, WABinary_1.getBinaryNodeChild)(profiles, "address"),
                        ...
                        businessHoursConfig = businessHours ? (0, WABinary_1.getBinaryNodeChildren)(businessHours, "business_hours_config") : void 0,
                        websiteStr = website?.content?.toString();
                    return {
                ...
                if (results) {
                    const verifiedResults = await Promise.all(results.list.filter(a => !!a.contact).map(async ({
                        contact: contact,
                        id: id,
                        lid: lid
                    }) => {
                ...
                if (result) {
                    ...
                    return {
                        ...
                        setAt: new Date(1e3 * +(status?.attrs.t || 0))
                    }
                }
            }, updateProfilePicture = async (jid, content) => {
                let targetJid;
                if (!jid) throw new boom_1.Boom("Illegal no-jid profile update. Please specify either your ID or the ID of the chat you wish to update");
                if ((0, WABinary_1.jidNormalizedUser)(jid) !== (0, WABinary_1.jidNormalizedUser)(authState.creds.me.id)) targetJid = (0, WABinary_1.jidNormalizedUser)(jid);
                const {
                    img: img
                ...
                await authState.keys.transaction(async () => {
                    ...
                    while (collectionsToHandle.size) {
                        ...
                        for (const name of collectionsToHandle) {
                            ...
                            } else state = (0, Utils_1.newLTHashState)();
                            states[name] = state;
                        ...
                        const result = await query({
                            ...
                            decoded = await (0, Utils_1.extractSyncdPatches)(result, config?.options);
                        for (const key in decoded) {
                            ...
                            try {
                                ...
                                const isIrrecoverableError = attemptsMap[name] >= MAX_SYNC_ATTEMPTS || error.output?.statusCode === 404 || error.name === "TypeError";
                                logger.info({
                                    name: name,
                                    error: error.stack
                                ...
                                if (isIrrecoverableError) collectionsToHandle.delete(name)
                            }
                        }
                    }
                });
                ...
                if (type === "available" || type === "unavailable") {
                    ...
                    const {
                        server: server
                    } = (0, WABinary_1.jidDecode)(toJid), isLid = server === "lid";
                    await sendNode({
                        tag: "chatstate",
                        attrs: {
            ...
            }, presenceSubscribe = (toJid, tcToken) => sendNode({
                tag: "presence",
                attrs: {
                    to: toJid,
                    id: generateMessageTag(),
                ...
                if (tag === "presence") presence = {
                    lastKnownPresence: attrs.type === "unavailable" ? "unavailable" : "available",
                    lastSeen: attrs.last && attrs.last !== "deny" ? +attrs.last : void 0
                };
                ...
                if (presence) ev.emit("presence.update", {
                    id: jid,
                    presences: {
                ...
                if (!myAppStateKeyId) throw new boom_1.Boom("App state key not present!", {
                    statusCode: 400
                });
                ...
                if (config.emitOwnEvents) {
                    ...
                    }], initial, getAppStateSyncKey, config.options, void 0, logger);
            ...
            }, star = (jid, messages, star) => chatModify({
                star: {
                    messages: messages,
                    star: star
                }
            }, jid), addChatLabel = (jid, labelId) => chatModify({
                addChatLabel: {
                    labelId: labelId
                }
            }, jid), removeChatLabel = (jid, labelId) => chatModify({
                removeChatLabel: {
                    labelId: labelId
                }
            }, jid), addMessageLabel = (jid, messageId, labelId) => chatModify({
                addMessageLabel: {
                    messageId: messageId,
                    labelId: labelId
                }
            }, jid), removeMessageLabel = (jid, messageId, labelId) => chatModify({
                removeMessageLabel: {
                    messageId: messageId,
                    labelId: labelId
                }
            }, jid), executeInitQueries = async () => {
                ...
                if (!!msg.pushName) {
                    ...
                    jid = (0, WABinary_1.jidNormalizedUser)(jid);
                    if (!msg.key.fromMe) ev.emit("contacts.update", [{
                        id: jid,
                        notify: msg.pushName,
                        verifiedName: msg.verifiedBizName
                    }]);
                    if (msg.key.fromMe && msg.pushName && authState.creds.me?.name !== msg.pushName) ev.emit("creds.update", {
                        me: {
                            ...authState.creds.me,
                            name: msg.pushName
                        }
        ...
        ws.on("CB:ib,,dirty", async node => {
            const {
                attrs: attrs
            } = (0, WABinary_1.getBinaryNodeChild)(node, "dirty"), type = attrs.type;
            switch (type) {
                case "account_sync":
                    if (attrs.timestamp) {
                        let {
